...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        # self.qkv(x) (B,197,192*3)
        # reshape (B,197,3,8,24)
        # permute (3,B,8,197,24)
...
class RPE_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., join_type='basic', num_patches=196):
        super().__init__()
        ...
        if self.join_type == 'LN':
            self.pe_norm = nn.LayerNorm(num_patches)
        # define a parameter table of relative position bias
        # 定义了一个(2*Wh-1 * 2*Ww-1)大小的表格，表格中的每个位置代表一种相对位置编码的值
        # 例如(0,1)下的值代表横坐标相差0，纵坐标相差1的两个token之间的位置编码值
    ...
    def forward(self, x):
        B, N, C = x.shape
        # self.qkv把(B,197,192)转为(B,197,192*3)
        # reshape成(B,197,3,8,192/8)=(B,197,3,8,24)
        # 重新排列维度成(3,B,8,197,24)
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # q的维度是(B,8,197,24)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
        # q@k.T的维度是(B,8,197,197)
        q = q * self.scale
        ...
        if self.join_type == 'LN':
            # attn维度[8192, 3, 196, 196]
            # relative_position_bias.unsqueeze(0)维度是[1, 3, 196, 196]
        ...
        # attn@v的维度是(B,8,197,24) transpose后为(B,197,8,24)
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class RPE_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, join_type='basic', num_patches=196):
        super().__init__()
        self.join_type = join_type
        self.norm1 = norm_layer(dim)
        self.attn = RPE_Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, 
            proj_drop=drop, join_type=join_type, num_patches=num_patches)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, join_type='basic', ind=None):
        super().__init__()
        self.join_type = join_type
        if self.join_type=='LaPE':
    ...
    def forward(self, x, pos_embed=None, ind=None, norm_pe=None):
        if self.join_type in ('basic','share','unshare'):
            x = x + self.drop_path(self.attn(self.norm1(x)))
        elif self.join_type=='LaPE':
            pos_embed = self.pe_norm(pos_embed)
            x = x + self.drop_path(self.attn(self.norm1(x)+pos_embed))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x, pos_embed
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, pe='learnable', 
                 join_type='basic'):
        super().__init__()
        self.pe = pe
        self.depth = depth
        ...
        if pe=='1D_sin':
            print('Using 1D sinusoidal position embedding.')
            ...
            print('Using 2D sinusoidal position embedding.')
            height = width = int(num_patches**0.5)
            pos_embed = self._positionalencoding2d(embed_dim,height,width)
            pos_embed = pos_embed.view(pos_embed.shape[0],-1).permute(1,0)
            # zero pe for cls token
            pos_embed = torch.cat((torch.zeros(1,embed_dim),pos_embed),0)
            pos_embed = torch.unsqueeze(pos_embed,0)
            self.pos_embed = nn.Parameter(pos_embed,requires_grad=False)
        elif pe=='learnable':
            if join_type != 'unshare':
                print('Using learnable position encoding.')
                self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
                trunc_normal_(self.pos_embed, std=.02)
            else:
                print('Using learnable position encoding for each layer.')
                for depth_i in range(depth):
                    pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
                    trunc_normal_(pos_embed, std=.02)
                    setattr(self, f"pos_embed{depth_i}", pos_embed)
            
        self.pos_drop = nn.Dropout(p=drop_rate)
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        
        if pe in ['1D_RPE', '2D_RPE']:
            self.blocks = nn.ModuleList([
                RPE_Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
                          qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], 
                          norm_layer=norm_layer, join_type=join_type, num_patches=num_patches)
                for i in range(depth)])
        else:
            self.blocks = nn.ModuleList([
                Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
                      qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], 
                      norm_layer=norm_layer, join_type=join_type)
                for i in range(depth)])
    ...
    def forward_features(self, x, **kwargs):
        ...
        if self.pe=='2D_RPE':
            ...
            for (ind, blk) in enumerate(self.blocks):
                x, pos_embed = blk(x, pos_embed=pos_embed, ind=ind, **kwargs)
        
        elif self.join_type=='share':
            x = self.pos_drop(x)
            for (ind,blk) in enumerate(self.blocks):
                x, _ = blk(x+self.pos_embed)
        
        elif self.join_type=='unshare':
            x = self.pos_drop(x)
            for (ind,blk) in enumerate(self.blocks):
                pos_embed_i = getattr(self, f"pos_embed{ind}")
                x, _ = blk(x+pos_embed_i)
